...
<head>
	...
	<script>  
		function showMessage(messageHTML, type) {
			var message = window.chatbox.appendChild(document.createElement("div"));
		...
		window.onload = function(){
			...
			var websocket = new WebSocket("ws://websocket.local:8090/php-socket.php"); 
			websocket.onopen = function(event) { 
				showMessage("Connection is established!", "chat-connection-ack");		
			}
			...
			websocket.onerror = function(event){
				showMessage("Problem due to some Error", "error");
			};
			websocket.onclose = function(event){
				showMessage("Connection Closed", "chat-connection-ack");
			}; 
		
			document.getElementById("frmChat").onsubmit = function(event){
				event.preventDefault();
				document.getElementById("chat-user").type = "hidden";		
